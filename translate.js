...
const post_translation = async (text, source, target) => {
	...
	return datastore.save({ key: key, excludeFromIndexes: [ 'text', 'translated_text' ], data: new_data }).then(() => {
		return key;
	});
};
...
router.post('/', cors(), (req, res) => {
	if (req.get('content-type') !== 'application/json') {
		res.status(415).send({ Error: 'Server only accepts accplication/json or text/plain data.' });
...
router.post('/ns', cors(), (req, res) => {
    if (req.get('content-type') !== 'application/json') {
		...
		if (valid) {
			translateText(req.body.text, req.body.source, req.body.target).then((translated_text) => {
                res.status(201).send({ translated: translated_text });
            });
		} else {
			res.status(400).send({ Error: err_msg });
		}
